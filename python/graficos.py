...
def graficar_superposicion_sens_ruido(titulo, datos_sensibilidad, datos_ruido, datos_temp):
    fig = go.Figure()
    s_max = max((np.max(d["y"]) for d in datos_sensibilidad.values() if len(d["y"]) > 0), default=1.0)
    r_max = max((np.max(d["y"]) for d in datos_ruido.values() if len(d["y"]) > 0), default=1.0)
    ...
    for disp in datos_sensibilidad.keys():
        ...
        if datos_temp and disp in datos_temp:
            fig.add_trace(go.Scatter(x=datos_temp[disp]["x"], y=np.abs(datos_temp[disp]["y"]), mode='markers+lines', name=f"{disp} (TC)", line=dict(dash='dot', color=color), marker=dict(symbol='triangle-up-open'), yaxis='y3'))
