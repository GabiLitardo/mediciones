...
# Variables auxiliares para compartir datos entre secciones si es necesario
sens_abs_t2 = None
...
if mostrar_sensibilidad:
    ...
    graficos.graficar_sensibilidad_fg(
        titulo="Sensibilidad FG tanda 1 (Tasa vs $I_D$ Normalizado)", 
        datos_sensibilidad=sens_norm_t1, 
        xlabel=r"Corriente normalizada $I_{D_{norm}}$ [$\mu$A]", 
        ylabel=r"Tasa de cambio normalizada [($\mu$A)/min]"
    )

    sens_norm_t2 = proc_sens.procesar_sensibilidad(["PFGIW1", "PFGIW2", "PFGIW3", "PFGIP2"], "FG_tanda2", normalizado=True)
    graficos.graficar_sensibilidad_fg(
        titulo="Sensibilidad FG tanda 2 (Tasa vs $I_D$ normalizado)", 
    ...
    graficos.graficar_sensibilidad_fg(
        titulo="Sensibilidad absoluta FG Tanda 1 (Tasa absoluta vs $I_D$ normalizado)", 
        datos_sensibilidad=sens_abs_t1, 
        xlabel=r"Corriente normalizada $I_{D_{norm}}$ [$\mu$A]", 
        ylabel=r"Tasa de cambio absoluta [$\mu$A/min]"
    )

    sens_abs_t2 = proc_sens.procesar_sensibilidad(["PFGIW1", "PFGIW2", "PFGIW3", "PFGIP2"], "FG_tanda2", normalizado=False)
    graficos.graficar_sensibilidad_fg(
        titulo="Sensibilidad absoluta FG Tanda 2 (Tasa absoluta vs $I_D$ normalizado)", 
...
if mostrar_ruido:
    ...
    evos = proc_ruido.obtener_evolucion_ruido(lista_dispositivos, corrientes_nominales, es_larga=False)
